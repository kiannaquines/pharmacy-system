...
import { fetchDashboardSummary, fetchProducts, fetchSales, fetchSuppliers } from "@/lib/api";
import { formatCurrency } from "@/lib/format";
import { cn } from "@/lib/utils";

const primaryNav = [
  { label: "Dashboard", icon: LayoutDashboard, active: true },
  { label: "Lifecycle", icon: Activity },
...
const documentNav = [
  ...
  { label: "Word Assistant", icon: LifeBuoy },
...
export default async function Home() {
  const [dashboard, products, suppliers, sales] = await Promise.all([
        ...
        <aside className="border-b border-border/70 bg-[#fbfbfa] px-5 py-6 lg:border-r lg:border-b-0">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-[2rem] font-semibold tracking-tight">Acme Inc.</h1>
            </div>
          </div>
          ...
          <div className="mt-12">
            <p className="px-3 text-sm font-medium text-muted-foreground">Documents</p>
        ...
        <section className="bg-[#fcfcfb]">
          <header className="flex items-center justify-between gap-4 border-b border-border/70 px-6 py-5 md:px-8">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3 border-l border-border pl-5">
                <h2 className="text-[2rem] font-semibold tracking-tight">Documents</h2>
              </div>
            </div>
            <Button variant="ghost" className="rounded-xl px-2 text-base font-medium">GitHub</Button>
          </header>
          ...
          <div className="space-y-8 p-6 md:p-8">
            <section className="grid gap-5 xl:grid-cols-4">
              <MiniStatCard
                title="Total Revenue"
                value={formatCurrency(totalRevenue || 1250)}
                note="Trending up this month"
                trend="↗ +12.5%"
              />
              <MiniStatCard
                title="New Customers"
                value={String(suppliers.length * 617 || 1234)}
                note="Down 20% this period"
                trend="↘ -20%"
              />
              <MiniStatCard
                title="Active Accounts"
                value={String(products.length * 22839 || 45678)}
                note="Strong user retention"
                trend="↗ +12.5%"
              />
              <MiniStatCard
                title="Growth Rate"
                value={`${Math.max(dashboard.totals.lowStock, 4.5)}%`}
            ...
            <section>
              <Card className="rounded-[32px] border border-border/70 bg-background shadow-sm shadow-black/[0.03]">
                <CardHeader className="pb-0">
                  <CardTitle className="text-[2rem] tracking-tight">Total Visitors</CardTitle>
                  <CardDescription className="text-lg">Total for the last 3 months</CardDescription>
                </CardHeader>
                <CardContent>
                  <AreaLikeChart />
                </CardContent>
